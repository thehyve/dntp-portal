...
module.exports = function () {
    ...
    this.Then(/^the form contains the following data\w*$/, function (fields, next) {
        ...
        for (var i in lines) {
            ...
            //work around for problems with resolving chai expect.
            Promise.resolve(expect(element(by.id(id)).getAttribute('value')).to.eventually.equal(content)).then(function (value) {
                if (lines[lines.length - 1].match(regex)[2] == value) {
                    next()
                }
            }, next);
        }
    ...
    this.Then(/^the static form contains\w*$/, function (fields, next) {
        var regex = /^(.+): (.+)\s*$/;
        ...
        lines.forEach(function (line) {
            ...
            Promise.resolve(element(by.cssContainingText('label.ng-binding', label)).element(by.xpath('..')).$$('.ng-binding')).then(function (el) {
                expect(el[1].getText()).to.eventually.equal(content).then(function (value) {
                    if (lines[lines.length - 1].match(regex)[2] == value) {
                        next()
                    }
                }, next);
