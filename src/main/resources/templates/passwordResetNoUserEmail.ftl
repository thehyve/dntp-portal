Geachte heer/mevrouw,

Een wachtwoordreset is aangevraagd voor uw emailadres, maar dit account is niet geregistreerd in de PALGA aanvraagportal.

Als u een account wilt aanmaken, klik dan op de volgende link:
${link}

Als u geen wachtwoordreset heeft aangevraagd kunt u dit bericht negeren.

Met vriendelijke groet,
Stichting PALGA
088-0402700 / aanvraag@palga.nl

--
Dit is een automatisch gegenereerd bericht.
Heeft u vragen, stuur dan een mail naar aanvraag@palga.nl.

=========================

Dear Sir/Madam,

A password reset was requested for your email address but this account is not registered in the PALGA request portal.

If you would like to create a new account, please click the following link:
  ${link}

If you did not request a password reset, you can ignore this e-mail.

With kind regards,
Stichting PALGA
088-0402700 / aanvraag@palga.nl

--
This is an automatically generated message.
If you have any questions, please send an email to aanvraag@palga.nl.
