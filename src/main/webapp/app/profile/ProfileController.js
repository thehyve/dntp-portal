...
angular.module('ProcessApp.controllers')
    .controller('ProfileController', ['$scope', '$timeout', 'Restangular', '$log',
        ...
        Restangular.one('api/profile').get().then(function (profile) {
            $scope.user = profile;
            Restangular.all('api/public/labs').getList().then(function (labs) {
                $scope.labs = labs;
                $scope.loaded = true;
            }, restError);
        }, restError);
        ...
        $scope.submitForm = function () {
            ...
            Restangular.one('api/profile').customPUT($scope.user).then(function () {
                $scope.submitted = false;
                $scope.success = true;
                $timeout(function () {
                    $scope.success = false;
